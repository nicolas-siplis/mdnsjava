...
public class LOCRecord extends Record {
  ...
  private long parsePosition(net.posick.DNS.Tokenizer st, String type) throws IOException {
  ...
  private long parseDouble(
          net.posick.DNS.Tokenizer st, String type, boolean required, long min, long max, long defaultValue)
      throws IOException {
    net.posick.DNS.Tokenizer.Token token = st.get();
    if (token.isEOL()) {
      if (required) {
        throw st.exception("Invalid LOC " + type);
      }
