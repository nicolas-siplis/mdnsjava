...
import net.posick.DNS.DNSSEC.Algorithm;
...
final class ValUtils {
  ...
  private final net.posick.DNS.dnssec.DnsSecVerifier verifier;
  ...
  public ValUtils() {
    this.verifier = new net.posick.DNS.dnssec.DnsSecVerifier();
   ...
   */
  public static void setCanonicalNsecOwner(net.posick.DNS.dnssec.SRRset set, RRSIGRecord sig) {
   ...
   */
  public static net.posick.DNS.dnssec.ResponseClassification classifyResponse(Message request, net.posick.DNS.dnssec.SMessage m) {
    ...
    if (m.getRcode() == Rcode.NXDOMAIN && m.getCount(Section.ANSWER) == 0) {
      return net.posick.DNS.dnssec.ResponseClassification.NAMEERROR;
    }
    ...
    if (!request.getHeader().getFlag(Flags.RD)
      ...
      for (RRset set : m.getSectionRRsets(Section.AUTHORITY)) {
        if (set.getType() == Type.SOA) {
          return net.posick.DNS.dnssec.ResponseClassification.NODATA;
        }

        if (set.getType() == Type.DS) {
          return net.posick.DNS.dnssec.ResponseClassification.REFERRAL;
        }
      ...
      return sawNs ? net.posick.DNS.dnssec.ResponseClassification.REFERRAL : net.posick.DNS.dnssec.ResponseClassification.NODATA;
    }
    ...
    if (m.getRcode() != Rcode.NOERROR && m.getRcode() != Rcode.NXDOMAIN) {
      return net.posick.DNS.dnssec.ResponseClassification.UNKNOWN;
    }
    ...
    if (qtype == Type.ANY) {
      return net.posick.DNS.dnssec.ResponseClassification.ANY;
    }

    boolean hadCname = false;
    for (RRset set : m.getSectionRRsets(Section.ANSWER)) {
      if (set.getType() == qtype) {
        return net.posick.DNS.dnssec.ResponseClassification.POSITIVE;
      }

      if (set.getType() == Type.CNAME || set.getType() == Type.DNAME) {
        hadCname = true;
        if (qtype == Type.DS) {
          return net.posick.DNS.dnssec.ResponseClassification.CNAME;
        }
      }
    }
    ...
    if (hadCname) {
      if (m.getRcode() == Rcode.NXDOMAIN) {
        return net.posick.DNS.dnssec.ResponseClassification.CNAME_NAMEERROR;
      } else {
        return net.posick.DNS.dnssec.ResponseClassification.CNAME_NODATA;
      }
    }
   ...
   */
  public net.posick.DNS.dnssec.KeyEntry verifyNewDNSKEYs(
          net.posick.DNS.dnssec.SRRset dnskeyRrset, net.posick.DNS.dnssec.SRRset dsRrset, long badKeyTTL, Instant date) {
    if (!atLeastOneDigestSupported(dsRrset)) {
      net.posick.DNS.dnssec.KeyEntry ke =
          net.posick.DNS.dnssec.KeyEntry.newNullKeyEntry(dsRrset.getName(), dsRrset.getDClass(), dsRrset.getTTL());
      ke.setBadReason(
          ExtendedErrorCodeOption.UNSUPPORTED_DS_DIGEST_TYPE,
          net.posick.DNS.dnssec.R.get("failed.ds.nodigest", dsRrset.getName()));
      return ke;
    }
    ...
    if (!atLeastOneSupportedAlgorithm(dsRrset)) {
      ...
      ke.setBadReason(
          ...
          net.posick.DNS.dnssec.R.get("failed.ds.noalg", dsRrset.getName()));
      return ke;
    }

    int favoriteDigestID = this.favoriteDSDigestID(dsRrset);
    net.posick.DNS.dnssec.KeyEntry ke = null;
    ...
    if (ke == null) {
      ke = net.posick.DNS.dnssec.KeyEntry.newBadKeyEntry(dsRrset.getName(), dsRrset.getDClass(), badKeyTTL);
      ke.setBadReason(ExtendedErrorCodeOption.DNSKEY_MISSING, net.posick.DNS.dnssec.R.get("dnskey.no_ds_match"));
    }
    return ke;
  }

  private net.posick.DNS.dnssec.KeyEntry getKeyEntry(net.posick.DNS.dnssec.SRRset dnskeyRrset, Instant date, DSRecord ds, DNSKEYRecord dnskey) {
    ...
    if (keyHash.length != dsHash.length) {
      ke = net.posick.DNS.dnssec.KeyEntry.newBadKeyEntry(ds.getName(), ds.getDClass(), ds.getTTL());
      ke.setBadReason(ExtendedErrorCodeOption.DNSSEC_BOGUS, net.posick.DNS.dnssec.R.get("dnskey.invalid"));
      return ke;
    }
    ...
    net.posick.DNS.dnssec.JustifiedSecStatus res = this.verifier.verify(dnskeyRrset, dnskey, date);
    switch (res.status) {
      case SECURE:
        dnskeyRrset.setSecurityStatus(net.posick.DNS.dnssec.SecurityStatus.SECURE);
        ke = net.posick.DNS.dnssec.KeyEntry.newKeyEntry(dnskeyRrset);
        break;
      case BOGUS:
        ke = net.posick.DNS.dnssec.KeyEntry.newBadKeyEntry(ds.getName(), ds.getDClass(), ds.getTTL());
   ...
   */
  int favoriteDSDigestID(net.posick.DNS.dnssec.SRRset dsset) {
   ...
   */
  public net.posick.DNS.dnssec.JustifiedSecStatus verifySRRset(net.posick.DNS.dnssec.SRRset rrset, net.posick.DNS.dnssec.SRRset keyRrset, Instant date) {
    if (rrset.getSecurityStatus() == net.posick.DNS.dnssec.SecurityStatus.SECURE) {
      log.trace(
          "RRset <{}/{}/{}> previously found to be SECURE",
          rrset.getName(),
          Type.string(rrset.getType()),
          DClass.string(rrset.getDClass()));
      return new net.posick.DNS.dnssec.JustifiedSecStatus(net.posick.DNS.dnssec.SecurityStatus.SECURE, -1, null);
    }

    net.posick.DNS.dnssec.JustifiedSecStatus res = this.verifier.verify(rrset, keyRrset, date);
   ...
   *     invalid name.
   */
  public static Name nsecWildcard(Name domain, net.posick.DNS.dnssec.SRRset set, NSECRecord nsec)
   ...
   */
  public static boolean nsecProvesNameError(net.posick.DNS.dnssec.SRRset set, NSECRecord nsec, Name qname) {
   ...
   * @return true if the NSEC proves the condition.
   */
  public static boolean nsecProvesNoWC(net.posick.DNS.dnssec.SRRset set, NSECRecord nsec, Name qname) {
  ...
  /**
   * Container for responses of {@link ValUtils#nsecProvesNodata(net.posick.DNS.dnssec.SRRset, NSECRecord, Name, int)}.
   */
  ...
  public static NsecProvesNodataResponse nsecProvesNodata(
          net.posick.DNS.dnssec.SRRset set, NSECRecord nsec, Name qname, int qtype) {
   ...
   */
  public net.posick.DNS.dnssec.JustifiedSecStatus nsecProvesNodataDsReply(
          Message request, net.posick.DNS.dnssec.SMessage response, net.posick.DNS.dnssec.SRRset keyRrset, Instant date) {
    ...
    net.posick.DNS.dnssec.SRRset nsecRrset = response.findRRset(qname, Type.NSEC, qclass, Section.AUTHORITY);
    if (nsecRrset != null) {
      // The NSEC must verify, first of all.
      net.posick.DNS.dnssec.JustifiedSecStatus res = this.verifySRRset(nsecRrset, keyRrset, date);
      if (res.status != net.posick.DNS.dnssec.SecurityStatus.SECURE) {
        return new net.posick.DNS.dnssec.JustifiedSecStatus(
            ...
            net.posick.DNS.dnssec.R.get("failed.ds.nsec", res.reason));
      }

      NSECRecord nsec = (NSECRecord) nsecRrset.first();
      net.posick.DNS.dnssec.SecurityStatus status = ValUtils.nsecProvesNoDS(nsec, qname);
      switch (status) {
        case INSECURE: // this wasn't a delegation point.
          return new net.posick.DNS.dnssec.JustifiedSecStatus(status, -1, net.posick.DNS.dnssec.R.get("failed.ds.nodelegation"));
        case SECURE: // this proved no DS.
          return new net.posick.DNS.dnssec.JustifiedSecStatus(status, -1, net.posick.DNS.dnssec.R.get("insecure.ds.nsec"));
        default: // something was wrong.
          return new net.posick.DNS.dnssec.JustifiedSecStatus(
              status, ExtendedErrorCodeOption.DNSSEC_BOGUS, net.posick.DNS.dnssec.R.get("failed.ds.nsec.hasdata"));
      }
    }
    ...
    for (net.posick.DNS.dnssec.SRRset set : response.getSectionRRsets(Section.AUTHORITY, Type.NSEC)) {
      net.posick.DNS.dnssec.JustifiedSecStatus res = this.verifySRRset(set, keyRrset, date);
      if (res.status != net.posick.DNS.dnssec.SecurityStatus.SECURE) {
        return new net.posick.DNS.dnssec.JustifiedSecStatus(res.status, res.edeReason, net.posick.DNS.dnssec.R.get("failed.ds.nsec.ent"));
      }
    ...
    if (hasValidNSEC) {
      if (ndp.wc != null) {
        net.posick.DNS.dnssec.SecurityStatus status = nsecProvesNoDS(wcNsec, qname);
        return new net.posick.DNS.dnssec.JustifiedSecStatus(
            status, ExtendedErrorCodeOption.NSEC_MISSING, net.posick.DNS.dnssec.R.get("failed.ds.nowildcardproof"));
      }

      return new net.posick.DNS.dnssec.JustifiedSecStatus(net.posick.DNS.dnssec.SecurityStatus.INSECURE, -1, net.posick.DNS.dnssec.R.get("insecure.ds.nsec.ent"));
    }

    return new net.posick.DNS.dnssec.JustifiedSecStatus(
        net.posick.DNS.dnssec.SecurityStatus.UNCHECKED,
   ...
   */
  public boolean hasSignedNsecs(net.posick.DNS.dnssec.SMessage message) {
    for (net.posick.DNS.dnssec.SRRset set : message.getSectionRRsets(Section.AUTHORITY)) {
   ...
   * @return {@link net.posick.DNS.dnssec.SecurityStatus#BOGUS} when the NSEC is from the child domain or indicates that
   *     there indeed is a DS record, {@link net.posick.DNS.dnssec.SecurityStatus#INSECURE} when there is not even a prove
   *     for a NS record, {@link net.posick.DNS.dnssec.SecurityStatus#SECURE} when there is no DS record.
   */
  public static net.posick.DNS.dnssec.SecurityStatus nsecProvesNoDS(NSECRecord nsec, Name qname) {
    ...
    if ((nsec.hasType(Type.SOA) && !Name.root.equals(qname)) || nsec.hasType(Type.DS)) {
      ...
      return net.posick.DNS.dnssec.SecurityStatus.BOGUS;
    }

    if (!nsec.hasType(Type.NS)) {
      // If there is no NS at this point at all, then this doesn't prove
      // anything one way or the other.
      return net.posick.DNS.dnssec.SecurityStatus.INSECURE;
    }
