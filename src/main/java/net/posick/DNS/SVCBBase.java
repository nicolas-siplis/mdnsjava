...
public abstract class SVCBBase extends Record {
  ...
  protected net.posick.DNS.Name targetName;
  ...
  protected SVCBBase(net.posick.DNS.Name name, int type, int dclass, long ttl) {
  ...
  public net.posick.DNS.Name getTargetName() {
    return targetName;
  }
  ...
  private static class ParameterMnemonic extends net.posick.DNS.Mnemonic {
    private final HashMap<Integer, Supplier<ParameterBase>> factories;

    public ParameterMnemonic() {
      super("SVCB/HTTPS Parameters", net.posick.DNS.Mnemonic.CASE_LOWER);
      setPrefix("key");
  ...
  public static class ParameterMandatory extends ParameterBase {
    ...
    public void fromWire(byte[] bytes) throws IOException {
      values.clear();
      net.posick.DNS.DNSInput in = new net.posick.DNS.DNSInput(bytes);
  ...
  protected void rrFromWire(DNSInput in) throws IOException {
    ...
    targetName = new net.posick.DNS.Name(in);
    svcParams.clear();
    while (in.remaining() >= 4) {
      int key = in.readU16();
  ...
  @Override
  protected void rdataFromString(net.posick.DNS.Tokenizer st, Name origin) throws IOException {
