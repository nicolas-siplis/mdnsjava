...
/**
 * An implementation of {@link net.posick.DNS.Resolver} that can send queries to multiple servers, sending the
 * queries multiple times if necessary.
 *
 * @see net.posick.DNS.Resolver
 * @author Brian Wellington
 */
@Slf4j
public class ExtendedResolver implements net.posick.DNS.Resolver {
  private static class Resolution {
    private final net.posick.DNS.Message query;
    ...
    Resolution(ExtendedResolver eres, net.posick.DNS.Message query) {
    ...
    private CompletionStage<net.posick.DNS.Message> send(Executor executorService) {
    ...
    private CompletionStage<net.posick.DNS.Message> startAsync(Executor executorService) {
      return send(executorService)
          .handle((result, ex) -> handle(result, ex, executorService))
          .thenCompose(Function.identity());
    }

    private CompletionStage<net.posick.DNS.Message> handle(
            net.posick.DNS.Message result, Throwable ex, Executor executorService) {
      ...
      if (ex != null) {
        ...
        if (endTime - System.nanoTime() < 0) {
          CompletableFuture<net.posick.DNS.Message> f = new CompletableFuture<>();
  ...
  private static class ResolverEntry {
    private final net.posick.DNS.Resolver resolver;
    private final AtomicInteger failures;

    ResolverEntry(net.posick.DNS.Resolver r) {
  ...
  /**
   * Creates a new Extended Resolver. The default {@link net.posick.DNS.ResolverConfig} is used to determine the
   * servers for which {@link net.posick.DNS.SimpleResolver}s are initialized. The timeout for each server is
  ...
  public ExtendedResolver() {
    ...
    resolvers.addAll(
        servers.stream()
            .map(
                server -> {
                  net.posick.DNS.Resolver r = new net.posick.DNS.SimpleResolver(server);
  ...
  /**
   * Creates a new instance with {@link net.posick.DNS.SimpleResolver}s. The timeout for each server is initialized
   * with {@link #DEFAULT_RESOLVER_TIMEOUT}.
   *
   * @param servers An array of server names or IP addresses for which {@link net.posick.DNS.SimpleResolver}s are
   *     initialized.
  ...
  public ExtendedResolver(String[] servers) throws UnknownHostException {
    for (String server : servers) {
      net.posick.DNS.Resolver r = new SimpleResolver(server);
      r.setTimeout(DEFAULT_RESOLVER_TIMEOUT);
   ...
   *
   * @param resolvers An array of pre-initialized {@link net.posick.DNS.Resolver}s.
   */
  public ExtendedResolver(net.posick.DNS.Resolver[] resolvers) {
   ...
   * ExtendedResolver}.
   *
   * @param resolvers An iterable of pre-initialized {@link net.posick.DNS.Resolver}s.
   */
  public ExtendedResolver(Iterable<net.posick.DNS.Resolver> resolvers) {
    for (net.posick.DNS.Resolver r : resolvers) {
   ...
   * individual {@link net.posick.DNS.Resolver}s. If the timeout expires, the {@link ExtendedResolver} simply stops
   ...
   */
  @Override
  public CompletionStage<net.posick.DNS.Message> sendAsync(net.posick.DNS.Message query) {
   ...
   */
  @Override
  public CompletionStage<net.posick.DNS.Message> sendAsync(Message query, Executor executor) {
  ...
  public net.posick.DNS.Resolver getResolver(int n) {
    if (n < resolvers.size()) {
  ...
  public net.posick.DNS.Resolver[] getResolvers() {
    return resolvers.stream().map(re -> re.resolver).toArray(net.posick.DNS.Resolver[]::new);
  }

  /** Adds a new resolver to be used by this ExtendedResolver */
  public void addResolver(net.posick.DNS.Resolver r) {
    resolvers.add(new ResolverEntry(r));
  }
