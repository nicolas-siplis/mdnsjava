...
public class SimpleResolver implements Resolver {
  ...
  private net.posick.DNS.TSIG tsig;
  ...
  @Override
  public void setEDNS(int version, int payloadSize, int flags, List<net.posick.DNS.EDNSOption> options) {
    switch (version) {
      case -1:
        queryOPT = null;
        break;
   ...
   * @since 3.2
   */
  public net.posick.DNS.TSIG getTSIGKey() {
    return tsig;
  }

  @Override
  public void setTSIGKey(net.posick.DNS.TSIG key) {
    tsig = key;
  }
  ...
  private net.posick.DNS.Message parseMessage(byte[] b) throws WireParseException {
    try {
      return new net.posick.DNS.Message(b);
    } catch (IOException e) {
  ...
  private void verifyTSIG(net.posick.DNS.Message query, net.posick.DNS.Message response, byte[] b, net.posick.DNS.TSIG tsig) {
    if (tsig == null) {
      return;
    }
  ...
  private void applyEDNS(net.posick.DNS.Message query) {
    if (queryOPT == null || query.getOPT() != null) {
      return;
    }
    query.addRecord(queryOPT, net.posick.DNS.Section.ADDITIONAL);
  }

  private int maxUDPSize(net.posick.DNS.Message query) {
   ...
   */
  @Override
  public CompletionStage<net.posick.DNS.Message> sendAsync(net.posick.DNS.Message query, Executor executor) {
    if (query.getHeader().getOpcode() == net.posick.DNS.Opcode.QUERY) {
    ...
    net.posick.DNS.Message ednsTsigQuery = query.clone();
  ...
  CompletableFuture<net.posick.DNS.Message> sendAsync(net.posick.DNS.Message query, boolean forceTcp, Executor executor) {
    int qid = query.getHeader().getID();
    byte[] out = query.toWire(net.posick.DNS.Message.MAXLENGTH);
    ...
    if (tcp) {
      ...
      result = net.posick.DNS.NioUdpClient.sendrecv(localAddress, address, out, udpSize, timeoutValue);
    }
    ...
    return result.thenComposeAsync(
        in -> {
          ...
          net.posick.DNS.Message response;
          try {
          ...
          if (!tcp && !ignoreTruncation && response.getHeader().getFlag(net.posick.DNS.Flags.TC)) {
            if (log.isTraceEnabled()) {
              log.trace(
  ...
  private net.posick.DNS.Message sendAXFR(net.posick.DNS.Message query) throws IOException {
    Name qname = query.getQuestion().getName();
    net.posick.DNS.ZoneTransferIn xfrin = ZoneTransferIn.newAXFR(qname, address, tsig);
    ...
    net.posick.DNS.Message response = new Message(query.getHeader().getID());
    response.getHeader().setFlag(net.posick.DNS.Flags.AA);
    response.getHeader().setFlag(Flags.QR);
    response.addRecord(query.getQuestion(), net.posick.DNS.Section.QUESTION);
