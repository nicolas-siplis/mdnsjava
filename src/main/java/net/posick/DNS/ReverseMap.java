...
public final class ReverseMap {
  ...
  private static final net.posick.DNS.Name inaddr4 = net.posick.DNS.Name.fromConstantString("in-addr.arpa.");
  private static final net.posick.DNS.Name inaddr6 = net.posick.DNS.Name.fromConstantString("ip6.arpa.");
   ...
   */
  public static net.posick.DNS.Name fromAddress(byte[] addr) {
    ...
    try {
      if (addr.length == 4) {
        return net.posick.DNS.Name.fromString(sb.toString(), inaddr4);
      } else {
        return net.posick.DNS.Name.fromString(sb.toString(), inaddr6);
      }
   ...
   * @return The name corresponding to the address in the reverse map.
   */
  public static net.posick.DNS.Name fromAddress(int[] addr) {
   ...
   * @return The name corresponding to the address in the reverse map.
   */
  public static net.posick.DNS.Name fromAddress(InetAddress addr) {
   ...
   */
  public static net.posick.DNS.Name fromAddress(String addr, int family) throws UnknownHostException {
    byte[] array = net.posick.DNS.Address.toByteArray(addr, family);
    if (array == null) {
   ...
   * @throws UnknownHostException The string does not contain a valid address.
   */
  public static net.posick.DNS.Name fromAddress(String addr) throws UnknownHostException {
    byte[] array = net.posick.DNS.Address.toByteArray(addr, net.posick.DNS.Address.IPv4);
    if (array == null) {
      array = net.posick.DNS.Address.toByteArray(addr, Address.IPv6);
    }
    if (array == null) {
      throw new UnknownHostException("Invalid IP address: " + addr);
    }
  ...
  public static InetAddress fromName(String name) throws UnknownHostException, TextParseException {
    return fromName(net.posick.DNS.Name.fromString(name));
  }
   ...
   * @since 3.1
   */
  public static InetAddress fromName(net.posick.DNS.Name name) throws UnknownHostException {
    if (name.labels() <= 3) {
    ...
    if (name.subdomain(inaddr4)) {
      net.posick.DNS.Name ip = name.relativize(inaddr4);
      if (ip.labels() > 4) {
