...
/**
 * The Response from a query to {@link Cache#lookupRecords(net.posick.DNS.Name, int, int)} or {@link
...
public class SetResponse {
   ...
   *
   * @see net.posick.DNS.DNAMERecord
   */
  ...
  private int type;
  private List<net.posick.DNS.RRset> data;

  private SetResponse() {}

  SetResponse(int type, net.posick.DNS.RRset rrset) {
    if (type < 0 || type > 6) {
  ...
  void addRRset(net.posick.DNS.RRset rrset) {
    if (data == null) {
      data = new ArrayList<>();
    }
    data.add(rrset);
  ...
  public List<net.posick.DNS.RRset> answers() {
    if (type != SUCCESSFUL) {
      return null;
    }
    return data;
  ...
  public net.posick.DNS.DNAMERecord getDNAME() {
